...
from geoseg.models.decoders.base import BaseDecodeHead
...
try:
    from geoseg.models.segman.csm_triton import CrossScanTriton, CrossMergeTriton
except:
...
class EfficientScan(torch.autograd.Function):
    ...
    def backward(ctx, grad_xs: torch.Tensor):  # [B, 4, H/w * W/w] -> [B, C, H, W]
        ...
        grad_x[:, 1, :, :, :] = (
            grad_xs[:, 1].reshape(B, C, newW, newH).transpose(dim0=2, dim1=3)
        )
        grad_x[:, 2, :, :, :] = grad_xs[:, 2].reshape(B, C, newH, newW)
        grad_x[:, 3, :, :, :] = (
            grad_xs[:, 3].reshape(B, C, newW, newH).transpose(dim0=2, dim1=3)
        )
...
class EfficientMerge(torch.autograd.Function):  # [B, 4, C, H/w * W/w] -> [B, C, H*W]
    ...
    def forward(ctx, ys: torch.Tensor, ori_h: int, ori_w: int, step_size=2):
        ...
        y[:, :, 1::step_size, ::step_size] = (
            ys[:, 1].reshape(B, C, W, H).transpose(dim0=2, dim1=3)
        )
        y[:, :, ::step_size, 1::step_size] = ys[:, 2].reshape(B, C, H, W)
        y[:, :, 1::step_size, 1::step_size] = (
            ys[:, 3].reshape(B, C, W, H).transpose(dim0=2, dim1=3)
        )
    ...
    def backward(ctx, grad_x: torch.Tensor):  # [B, C, H*W] -> [B, 4, C, H/w * W/w]
        ...
        grad_xs[:, 1] = grad_x.transpose(dim0=2, dim1=3)[
            :, :, ::step_size, 1::step_size
        ].reshape(B, C, -1)
        grad_xs[:, 2] = grad_x[:, :, ::step_size, 1::step_size].reshape(B, C, -1)
        grad_xs[:, 3] = grad_x.transpose(dim0=2, dim1=3)[
...
def cross_scan_continuous(x, num_scans=4, split=False):
    ...
    if split and C > 1:
        ...
        for i in range(paths_tb.size(0)):
            xs[:, i + num_scans // 2] = torch.index_select(
                x.flatten(-2, -1), -1, paths_tb[i]
            )
...
def cross_merge_continuous(ys, paths_lr, paths_tb, reverse_lr, reverse_tb, split=False):
    ...
    corresponding_scan_paths = torch.concat([reverse_lr, reverse_tb], dim=0).view(
        1, 1, 1, K, L
    )
...
class LayerScale(nn.Module):
    ...
    def extra_repr(self) -> str:
        return "{dim}, init_value={init_value}, bias={enable_bias}".format(
            **self.__dict__
        )
...
class VSSM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj_weight = nn.Parameter(
            torch.stack([t.weight for t in self.x_proj], dim=0).view(-1, d_inner, 1)
        )
        del self.x_proj
        ...
        self.A_logs = self.A_log_init(
            d_state, d_inner, copies=k_groups, merge=True
        )  # (K * D, N)
    ...
    def forward(self, x, to_dtype=False, force_fp32=False):
        ...
        if self.expansion_ratio != 1:
            xs = (
                self.xproj(xs.permute(0, 1, 3, 2).contiguous())
                .permute(0, 1, 3, 2)
                .contiguous()
            )
        xs = xs.reshape(B, -1, L)
        ...
        dts = F.conv1d(
            dts.reshape(B, -1, L), dt_projs_weight.reshape(K * D, -1, 1), groups=K
        )
        ...
        ys = self._selective_scan(
            xs, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus=True, ssoflex=True
        )
        ...
        if self.expansion_ratio != 1:
            y = (
                self.yproj(y.permute(0, 3, 2, 1).contiguous())
                .permute(0, 3, 2, 1)
                .contiguous()
            )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.lepe = nn.Conv2d(
            embed_dim, embed_dim, kernel_size=5, padding=2, groups=embed_dim
        )
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "rpb"):
            for item in self.rpb:
                nn.init.zeros_(
                    item
                )  # which better? nn.init.trunc_normal_(item, std=0.02)
    ...
    def forward(self, x, pos_enc):
        ...
        x = na2d_av(
            attn, v, kernel_size=toodd(self.window_size), dilation=self.window_dilation
        )

        x = rearrange(
            x, "b n_h h w c -> b (n_h c) h w", n_h=self.num_heads
        ).contiguous()
        x = x + lepe
        x = self.proj(x)
...
class FFN(nn.Module):
    def __init__(
        ...
        self.dwconv = nn.Conv2d(
            ffn_dim, ffn_dim, kernel_size=3, padding=1, groups=ffn_dim
        )
...
class SegMANDecoder(BaseDecodeHead):
    def __init__(
        ...
        **kwargs,
        ...
        self.reduce_channels = nn.ModuleList(
            [
                ConvModule(
                    in_channels=self.embed_dim * 4 * (2**i),
                    out_channels=self.embed_dim,
                    kernel_size=1,
                    norm_cfg=dict(type="SyncBN", requires_grad=True),
                )
    ...
    def forward_mlp_decoder(self, inputs):
        ...
        _c4 = resize(
            _c4, size=inputs[1].size()[2:], mode="bilinear", align_corners=False
        ).contiguous()
        _c3 = resize(
            _c3, size=inputs[1].size()[2:], mode="bilinear", align_corners=False
        ).contiguous()
