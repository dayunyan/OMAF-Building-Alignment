...
from natten.flops import add_natten_handle  # qk_2d_rpb_flop, av_2d_flop,
...
class Block(nn.Module):
    ...
    def forward(self, x, pos_enc):
        ...
        x = x + self.cpe1(x)
        x = x + self.drop_path(
            self.layer_scale1(self.token_mixer(self.norm1(x), pos_enc))
        )
        x = x + self.cpe2(x)
...
class SegMANEncoder(nn.Module):
    ...
    def flops(self, shape=(3, 224, 224), batch_size=128):
        ...
        input = torch.randn(
            (batch_size, *shape), device=next(model.parameters()).device
        )
