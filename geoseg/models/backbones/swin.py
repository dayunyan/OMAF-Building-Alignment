...
from tools.embed import PatchEmbed, PatchMerging
...
class swin_l(SwinTransformer):
    """
    Swin-L (Large) configuration.
    """

    def __init__(self, **kwargs):
        super(swin_l, self).__init__(
            embed_dims=192,
            depths=(2, 2, 18, 2),
...
def get_swin(
    ...
    """
    Helper function to instantiate Swin Transformer and load pretrained weights.
    """
    series = variety.lower()

    # 检查 variety 是否有效
    if series not in ["t", "s", "b", "l"]:
        raise ValueError(
            f"Unknown Swin variety: {variety}. Expected 't', 's', 'b', or 'l'."
        )

    # 使用 eval 动态实例化模型 (仿照 get_mit 示例)
    model_type = f"swin_{series}()"
    swin_net = eval(model_type)

    # 加载预训练权重
    if pretrain:
        print(f"Loading pretrained weights from: {pretrain}")
        try:
            state_dict = torch.load(pretrain, map_location="cpu")

            # 权重字典可能嵌套在 'state_dict' 或 'model' 键中
            ...
            for k, v in weights.items():
                ...
                # 2. 移除 'backbone.' 前缀
                if k.startswith("backbone."):
                    processed_weights[k.replace("backbone.", "")] = v
                # 3. 移除 'module.' 前缀 (DDP/DP 训练时产生)
                elif k.startswith("module."):
                    processed_weights[k.replace("module.", "")] = v
                # 4. 保留其他键
                else:
                    processed_weights[k] = v

            # 加载处理后的权重
            # SwinTransformer 类没有 'head'，所以 strict=False 是必须的
            missing_keys, unexpected_keys = swin_net.load_state_dict(
                processed_weights, strict=False
            )
            ...
            del state_dict, weights, processed_weights
