...
class VWA(NonLocal2d):
    ...
    def forward(self, query, context_k, context_v):
        ...
        y = (
            y.permute(0, 2, 1)
            .contiguous()
            .reshape(n, self.inter_channels, *query.size()[2:])
        )
...
class VWHead(BaseDecodeHead):
    ...
    def vwformer(self, _c):
        ...
        for j, r in enumerate(self.kernel):
            ...
            query = rearrange(
                _c, "b c (nh ph) (nw pw) -> (b nh nw) c ph pw", nh=nh, nw=nw
            )
            ...
            context = rearrange(
                context, "b c (ph rh) (pw rw) -> b (rh rw c) ph pw", rh=rh, rw=rw
            )

            _output = getattr(self, f"attn")[j](query, context, context)
            _output = rearrange(
                _output, "(b nh nw) c ph pw -> b c (nh ph) (nw pw)", nh=nh, nw=nw
            )
    ...
    def low_level(self, _c, c1):
        ...
        output = resize(_c, size=c1.size()[2:], mode="bilinear", align_corners=False)
...
class VWCityHead(BaseDecodeHead):  # vw for cityscapes
    def __init__(self, short_cut, nheads, **kwargs):
        ...
        self.linear_c4 = Conv2d(
            in_channels=feature_channels[3], out_channels=embed_dim, kernel_size=1
        )
        self.linear_c3 = Conv2d(
            in_channels=feature_channels[2], out_channels=embed_dim, kernel_size=1
        )
        self.linear_c2 = Conv2d(
            in_channels=feature_channels[1], out_channels=embed_dim, kernel_size=1
        )
        self.linear_c1 = Conv2d(
            in_channels=feature_channels[0], out_channels=48, kernel_size=1
        )
        self.linear_fuse = Conv2d(
            in_channels=embed_dim * 3, out_channels=embed_dim, kernel_size=1
        )
        self.short_path = Conv2d(
        ...
        self.cat = Conv2d(
            in_channels=embed_dim * 4, out_channels=embed_dim, kernel_size=1
        )
        self.low_level_fuse = Conv2d(
            in_channels=embed_dim + 48, out_channels=embed_dim, kernel_size=1
        )
        self.attn = nn.Sequential(
            *[
                nn.MultiheadAttention(embed_dim, nheads, batch_first=False)
                for _ in [2, 4, 8]
            ]
        )
    ...
    def forward_features(self, inputs):
        ...
        ph, pw = h // nh, w // nw
        query = rearrange(
            _c, "b c (nh ph) (nw pw) -> (ph pw) (b nh nw) c", nh=nh, nw=nw
        )
        ...
        for j, r in enumerate([2, 4, 8]):
            ...
            context = F.unfold(context, kernel_size=(ph * rh, pw * rw), stride=(ph, pw))
            context = rearrange(
                context,
                "b (c ph pw) (nh nw) -> (b nh nw) c ph pw",
                ph=ph * rh,
            ...
            context = rearrange(
                context, "b c (ph nh) (pw nw) -> (ph pw) b (nh nw c)", nh=rh, nw=rw
            )
            _output, _ = self.attn[j](query, context, context)
            _output = rearrange(
                _output,
                "(ph pw) (b nh nw) c -> b c (nh ph) (nw pw)",
                nh=nh,
                nw=nw,
                ph=ph,
                pw=pw,
