...
class Supervision_Train(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if mask.dim() == 3:
            inputs = torch.concat((img, mask.unsqueeze(1)), dim=1)
        else:
            inputs = torch.concat((img, mask), dim=1)

        output = self.net(inputs)
        loss = self.loss(output["logits"], align_conf)
        align_mask = (align_conf > 0).long()

        pre_mask = nn.Softmax(dim=1)(output["logits"]).argmax(dim=1)
        for i in range(mask.shape[0]):
            self.metrics_train.add_batch(
                align_mask[i].cpu().numpy(), pre_mask[i].cpu().numpy()
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (self.current_epoch + 1) % self.config.check_val_every_n_epoch == 0:
            for i in range(img.shape[0]):
                ...
                visualize_masks(
                    pre_mask[i],
                    torch.where(align_conf[i] > 0, 1, 0),
                    os.path.join(
                        self.config.visualize_name, f"epoch_{self.current_epoch}"
                    ),
                    f"{batch['img_id'][i]}_PAAlign",
                )
                # visualize_aligned_mask(
                #     img[i],
                #     pre_mask[i],
